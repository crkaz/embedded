...
char* get_time_bit_as_string(char b) {
    ...
    return convert_bcd_to_string(t);
}



char* get_time_as_string() {
    char hour[2]; hour = get_time_bit_as_string(HOUR);
    char min[2]; min = get_time_bit_as_string(MIN);
    char sec[2]; sec = get_time_bit_as_string(SEC);

    char time[9] = {hour[0], hour[1], ':', min[0], min[1], ':', sec[0], sec[1], '\0'};
